...
var mergeXWith = function mergeXWith(collision) {
    return function (own) {
        return function (src) {
            return function (tgt) {
                var _ref5 = own ? [whenHas, ifHas] : [whenHasIn, ifHasIn],
...
var getInfo = function getInfo(merger) {
    return merger === mergeToM ? ['to', true, true] : merger === _index2.default.mergeToM ? ['to', true, true] : merger === mergeM ? ['from', true, true] : merger === _index2.default.mergeM ? ['from', true, true] : merger === mergeTo ? ['to', false, true] : merger === _index2.default.mergeTo ? ['to', false, true] : merger === merge ? ['from', false, true] : merger === _index2.default.merge ? ['from', false, true] : merger === mergeToInM ? ['to', true, false] : merger === _index2.default.mergeToInM ? ['to', true, false] : merger === mergeInM ? ['from', true, false] : merger === _index2.default.mergeInM ? ['from', true, false] : merger === mergeToIn ? ['to', false, false] : merger === _index2.default.mergeToIn ? ['to', false, false] : merger === mergeIn ? ['from', false, false] : merger === _index2.default.mergeIn ? ['from', false, false] : (0, _index.die)('Unrecognised merge function');
};
        ...
        return function (a) {
            return function (b) {
                var _getInfo = getInfo(merger),
                    _getInfo2 = _slicedToArray(_getInfo, 3),
                    dir = _getInfo2[0],
                    mutable = _getInfo2[1],
                    own = _getInfo2[2];

                var _ref7 = dir === 'to' ? [b, a] : [a, b],
                    _ref8 = _slicedToArray(_ref7, 2),
                    src = _ref8[0],
                    tgt = _ref8[1];

                var tgtM = mutable ? tgt : dir === 'to' ? merger({})(tgt) : merger(tgt)({});
...
var mergeToWhenM = exports.mergeToWhenM = function mergeToWhenM(f) {
    return function (tgt) {
        return function (src) {
            for (var i in src) {
                if (hasOwn.call(src, i) && f(src[i], tgt[i])) tgt[i] = src[i];
            }return tgt;
        };
    };
};
...
// --- tests `f` for truthiness.
var mergeToWhen = exports.mergeToWhen = function mergeToWhen(f) {
    return function (tgt) {
        return function (src) {
            var a = mergeToM({})(tgt);
...
// --- tests `f` for truthiness.
var mergeWhenM = exports.mergeWhenM = function mergeWhenM(f) {
    return function (src) {
        return function (tgt) {
            return mergeToWhenM(f)(tgt)(src);
        };
    };
};

// --- tests `f` for truthiness.
var mergeWhen = exports.mergeWhen = function mergeWhen(f) {
    return function (src) {
        return function (tgt) {
            var a = mergeToM({})(tgt);
            return mergeToWhenM(f)(a)(src);
...
var asterisk = exports.asterisk = function asterisk(fs) {
    return function (xs) {
        ...
        try {
            for (var _iterator4 = fs[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                var f = _step4.value;
...
var mergeMixinPreM = function mergeMixinPreM(mixin) {
    return function (proto) {
        var chooseTarget = function chooseTarget(src, tgt) {
...
exports.default = {
    ...
    mergeToWhenM: mergeToWhenM, mergeWhenM: mergeWhenM,
    mergeToWhen: mergeToWhen, mergeWhen: mergeWhen,
    mergeToInM: mergeToInM, mergeInM: mergeInM, mergeToIn: mergeToIn, mergeIn: mergeIn,
    mergeWith: mergeWith,
