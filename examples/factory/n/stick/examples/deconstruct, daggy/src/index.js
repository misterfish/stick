...
const laatsNO = curry ((fs, o) => laats | passN ([
    o | always,
    ... fs,
]))
...
export const show = deconstruct (({ brightness, }) => brightness | cond ([
    'bright' | eq | guardA (brightMapFold >> log),
    otherwise | guardA (dullMapFold >> log),
]))

export const show1 = laatsO ([
    prop ('brightness'),
    (_, brightness) => brightness | cond ([
        'bright' | eq | guardA (brightMapFold >> log),
        otherwise | guardA (dullMapFold >> log),
    ]),
    (gip, _, f) => gip | f,
